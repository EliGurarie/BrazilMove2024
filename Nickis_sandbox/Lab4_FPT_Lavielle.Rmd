...
We can use Base R to visualize the track of this individual in space and time:


```{r}
ggplot(data=elk_example_data, aes(x=lon, y=lat)) +
  geom_path(size=0.5) +
  theme_classic()
```
...
First Passage Time (FPT) can be a useful paramter to describe scales of behavior along a movement track. 
...
2 - convert our data to be SpatialPoints format (from another spatial package, sp)
...
We can bind our FPT values to our data as a column by extacting it from our FPT object.

We can plot these values over our latitude values by scaling both of them (`scale` function). Periods of higher FPT seem to mostly correspond to the distinct patches of possible "residential" behavior.
...
Lavielle’s method identifies break points in a timeseries by applying a moving window over the timeseries. The optimal number of segments, k, in the timeseries is then determined by minimizing the "penalized contrast function" (Lavielle 2005). Break points are determined using the mean, variance, or mean AND variance along the timeseries.

WE will use the same example dataset as previous, to compare results.
...
We can use the visualizations of the track to inform our decisions on these parameters. Here, we will use a minimum of 20 locations, max of 5 segments, and the mean to determine segments by latitude.
