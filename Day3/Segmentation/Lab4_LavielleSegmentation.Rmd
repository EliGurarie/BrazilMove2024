...
Lavielle’s method identifies behavioral break points by applying a moving window over the timeseries. The optimal number of segments, *k*, in the timeseries is then determined by minimizing the "penalized contrast function" (Lavielle 2005). Break points are determined using the mean, variance, or mean AND variance along the timeseries of the variable of interest (e.g., speed).  The assumption is that in each phase, the model for the data is:
...
How do the speeds compare across htese phases? 
