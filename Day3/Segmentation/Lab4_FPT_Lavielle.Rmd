...
Lavielle’s method identifies behavioral break points by applying a moving window over the timeseries. The optimal number of segments, k, in the timeseries is then determined by minimizing the "penalized contrast function" (Lavielle 2005). Break points are determined using the mean, variance, or mean AND variance along the timeseries of the variable of interest (e.g., speed).
...
```{r}
breakpoints_df <- data.frame(Row_ID = breakpoints, segment= 1:length(breakpoints))
elk_example_data$Row_ID <- cumsum(rep(1, nrow(elk_example_data)))
elk_example_data_lav <- merge(elk_example_data, 
                              breakpoints_df, by="Row_ID", 
                              all.x=TRUE)
...
```{r}
ggplot(data=elk_example_data_lav, aes(x=datetime, y=lat)) +
  geom_path(col = "grey")+
  geom_path(aes(color = factor(segment))) + 
  theme_classic()
```
