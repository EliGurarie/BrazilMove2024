...
```{r SL}
elk_movestats <- elk_df |> ddply("id", getMoveStats)
```

```{r, echo = FALSE}
elk_movestats |>  head() |> kable()
```


```{r}
ggplot(elk_movestats, aes(doy, MR)) +
    geom_smooth() + 
  ylab("Movement Rate (km/h")  + scale_y_sqrt() + 
  ggtitle("Overall movement rates by day of year")
```

```{r}
n.ids <-  elk_movestats$id |> table()
ids <- names(n.ids)[n.ids > 1000]

ggplot(elk_movestats  |> subset(id %in% ids), aes(doy, MR, col = id)) +
    geom_smooth() + 
  ylab("Movement Rate (km/h")  + scale_y_sqrt() + 
  ggtitle("Individual movement rates by day of year") + 
  theme_classic()
```

```{r}
ggplot(elk_movestats, aes(Hour, MR)) +
    geom_smooth() + 
  ylab("Movement Rate (km/h")  + scale_y_sqrt() + 
  ggtitle("Overall movement rates by time of day")
```


# Final version

The code we developed in class is below, and works very similarly to the code above
...
myelk <- elk_df |> subset(id == 4049)

augmentDF <- function(df){
  df |> mutate(Month = month(datetime),
   	doy = yday(datetime),
    	hour = hour(datetime),
    	Z = X + 1i*Y,
    	Step = c(NA, diff(Z)),
...
elk_df2 <- elk_df |> group_by(id) |> augmentDF()

# seasonal patterns

myelk <- elk_df2 |> subset(id == 4049)

  ggplot(myelk, aes(x = factor(hour), y = Speed)) +
	geom_boxplot()
  ggplot(myelk |> na.omit(),
     	aes(x = doy, y = Speed)) +
	geom_smooth
...
elk_summary<-elk_df2 |> group_by(id) |> dplyr::summarize(
  n.obs = length(id),
  start = min(datetime),
  end = max(datetime),
  duration = difftime(end, start, units = "days"),
  total.distance = sum(StepLength, na.rm=TRUE),
  speed.median = median(Speed, na.rm = TRUE),
  speed.q25 = quantile(Speed, .25, na.rm = TRUE),
  speed.q75 = quantile(Speed, .75, na.rm = TRUE),
  fix.rate.median = median(dTime, na.rm  = TRUE),
  fix.rate.min = min(dTime, na.rm  = TRUE),
  fix.rate.max = max(dTime, na.rm  = TRUE)
) |> data.frame()
 


ggplot(elk_summary, aes(y = id, xmin = start, xmax = end)) +
     	geom_linerange(size = 2) +
  ggtitle("Elk monitoring times")
