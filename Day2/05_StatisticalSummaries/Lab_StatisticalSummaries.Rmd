...
The `diff()` function calculates differences among subsequent elements of a vector, or, for time (POSIX) data, the `difftime(t1, t2)` function will compute the difference between two times. 
...
```
With `difftime` and `diff` you can specify the unit (choose from: `"auto", "secs", "mins", "hours", "days", "weeks"`):

```{r}
diff(range(GP1$datetime), units = "hours")
...
```{r SL}
elk_movestats <- elk_df |> plyr::ddply("id", getMoveStats)
```
